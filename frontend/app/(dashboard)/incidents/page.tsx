...
export default function IncidentsPage() {
  ...
  // Test incident simulation mutation - local simulation only
  const testIncidentMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await fetch(`${apiUrl}/webhook/test/simulate-incident`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.detail || 'Failed to simulate test incident');
      }
    ...
    onSuccess: (data) => {
      ...
      if (data.status === 'success') {
        toast.success('Test incident simulated successfully!', {
          description: 'Local incident simulation completed - AI agent processing initiated.',
        });
      }
      queryClient.invalidateQueries({ queryKey: queryKeys.incidents() });
    },
    onError: (error: Error) => {
      setShowTestConfirmDialog(false);
      setTestConfirmStep(1);
      toast.error('Failed to simulate test incident', {
    ...
    <section className="flex-1 p-4 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {testIncidentMutation.isPending ? (
              <>
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-orange-600">
              ...
              {testConfirmStep === 1 ? 'Simulate Test Incident' : 'Final Confirmation'}
            </DialogTitle>
            <DialogDescription>
              {testConfirmStep === 1
                ? 'This will create a local test incident for AI agent processing'
                : 'Are you absolutely sure? This will create a test incident.'
              }
            </DialogDescription>
          </DialogHeader>
          ...
          {testConfirmStep === 1 ? (
              ...
              <Alert className="border-orange-200 bg-orange-50">
                ...
                <AlertDescription className="text-orange-700 space-y-2">
                  <ul className="list-disc pl-4 space-y-1 mt-2">
                    <li>A local test incident will be simulated</li>
                    <li>The AI agent will analyze and generate a response</li>
                    <li>No external services will be contacted</li>
                    <li>This is safe for development and testing</li>
                  </ul>
                </AlertDescription>
              </Alert>
              <div className="bg-gray-100 rounded-lg p-4">
                <p className="text-sm font-medium text-gray-700 mb-2">Test Incident Details:</p>
                <ul className="text-sm text-gray-600 space-y-1">
                  <li><strong>Type:</strong> Kubernetes Pod CrashLoopBackOff</li>
                  <li><strong>Severity:</strong> Warning</li>
                  <li><strong>Source:</strong> AutoSRE Test Button</li>
                  <li><strong>Mode:</strong> Local Simulation</li>
                </ul>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              <Alert className="border-blue-200 bg-blue-50">
                ...
                <AlertTitle className="text-blue-800">Confirmation Required</AlertTitle>
                <AlertDescription className="text-blue-700">
                  This will create a local test incident for AI agent development and testing.
                </AlertDescription>
              </Alert>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
                <p className="text-blue-800 font-medium">
                  Click &quot;Simulate Test Incident&quot; to proceed
                </p>
              </div>
            </div>
          )}
