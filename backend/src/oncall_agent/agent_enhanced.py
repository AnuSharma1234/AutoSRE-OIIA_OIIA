...
# JIRA integration via SuperPlane Canvas - no direct integration needed
...
class EnhancedOncallAgent:
    ...
    def __init__(self, ai_mode: AIMode = AIMode.PLAN):
        ...
        # JIRA integration is handled via SuperPlane Canvas - no direct integration needed
    ...
    async def handle_pager_alert(self, alert: PagerAlert, auto_remediate: bool = None) -> dict[str, Any]:
        ...
        try:
            ...
            if should_remediate and resolution_actions and self.agent_executor:
                ...
                # SuperPlane handles incident acknowledgment automatically via Canvas
                self.logger.info("🔄 SuperPlane Canvas managing incident lifecycle")
                ...
                if self.ai_mode == AIMode.YOLO or (execution_results['actions_failed'] == 0 and execution_results['actions_successful'] > 0):
                    ...
                    try:
                        # SuperPlane Canvas handles incident resolution automatically
                        self.logger.info(f"✅ SuperPlane incident {alert.alert_id} handled - resolution documented in JIRA")
                        
                        # Send resolution log to frontend
                        from .api.log_streaming import log_stream_manager
                        await log_stream_manager.log_success(
                            f"✅ Incident resolved: {alert.description[:50]}... - JIRA ticket created",
                            incident_id=alert.alert_id,
                            stage="incident_resolved",
                            progress=1.0,
                        )
                    except Exception as e:
                        self.logger.error(f"❌ Error sending resolution log: {e}")
            else:
