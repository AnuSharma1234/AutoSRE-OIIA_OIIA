...
- Log all operations clearly
    ...
    async def disconnect(self) -> None:
        ...
        try:
            if self.mcp_tools:
                # Suppress MCP client shutdown errors that don't affect functionality
                try:
                    await self.mcp_tools.close()
                except (RuntimeError, BaseExceptionGroup) as e:
                    # These are common MCP client shutdown errors that can be safely ignored
                    if "cancel scope" in str(e) or "unhandled errors in a TaskGroup" in str(e):
                        self.logger.debug(f"Suppressed MCP shutdown error (harmless): {e}")
                    else:
                        self.logger.warning(f"MCP shutdown warning: {e}")
                except Exception as e:
                    self.logger.warning(f"MCP shutdown error: {e}")
                finally:
                    self.mcp_tools = None
