...
class Config(BaseSettings):
    ...
    use_gemini: bool = Field(True, env="USE_GEMINI")

    # LiteLLM settings (secondary)
    ...
    # JIRA integration settings (replaces Notion/Grafana)
    jira_enabled: bool = Field(True, env="JIRA_ENABLED")
    jira_url: str = Field("https://auto-sre.atlassian.net", env="JIRA_URL")
    jira_username: str = Field("demo@auto-sre.com", env="JIRA_USERNAME")
    jira_api_token: str = Field("demo-token", env="JIRA_API_TOKEN")
    jira_project_key: str = Field("ASRE", env="JIRA_PROJECT_KEY")
    ...
    # SuperPlane integration settings (replaces PagerDuty)
    superplane_enabled: bool = Field(True, env="SUPERPLANE_ENABLED")
    superplane_webhook_secret: str | None = Field(None, env="SUPERPLANE_WEBHOOK_SECRET")
